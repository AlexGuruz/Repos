...
def test_projection_main_dedupes_duplicate_order_items(monkeypatch, tmp_path):
    ...
    def fake_fetch_chunk(**kwargs):
        return [dict(line), dict(line)], kwargs["oi_query"]

    def fake_validate_and_normalize(*, raw_json, **_kwargs):
        edges = raw_json["data"]["findOrderItems"]["edges"]
        validated_nodes.extend(edge["node"] for edge in edges)
        return {"ok": True, "report_path": str(tmp_path / "validation.json"), "errors": []}
    ...
    assert "- **Unique order lines counted:** 1" in out.read_text(encoding="utf-8")
