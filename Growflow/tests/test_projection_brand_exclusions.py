...
def test_projection_main_dedupes_duplicate_order_items(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: {"ok": True, "report_path": "mock-validation.json"},
    )
    ...
    assert "| Edibles | $100.00 | 100.00% | $100.00 |" in report
    assert "| Edibles | $100.00 | $100.00 | 100.00% | 100.00% |" in report
