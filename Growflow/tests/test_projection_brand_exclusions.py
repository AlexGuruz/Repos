...
def test_projection_main_dedupes_duplicate_order_items(tmp_path, monkeypatch):
    ...
    node = {
        ...
        "GrossPrice": 10_000,
        "COG": 4_000,
        "ProductCategory": {"Name": "Cartridge"},
        "Product": {
            "objectId": "product-1",
            "Name": "Acme Cartridge",
            "Brand": {"Name": "Acme"},
    ...
    def fake_fetch_chunk(**_kwargs):
        return [node, dict(node)], _mod.ORDER_ITEMS_QUERY

    def fake_validate_and_normalize(*, raw_json, **_kwargs):
        validation_counts.append(len(raw_json["data"]["findOrderItems"]["edges"]))
        return {"ok": True, "report_path": str(tmp_path / "validation.json")}

    out = tmp_path / "projection.md"
    monkeypatch.setenv("GROWFLOW_SALES_TZ", "UTC")
