...
def test_resolve_approval_executes_supervisor_payload_when_approved():
    ...
    with patch.object(events, "list_pending", return_value=[("approval-7", pending)]), \
         patch.object(events, "_resolve_queue", return_value=True), \
         patch("services.supervisor_bridge.execute_approved", new_callable=AsyncMock) as execute_supervisor:
        execute_supervisor.return_value = {"ok": True, "act_id": "ACT-SUP"}
        client = _client()
        response = client.post("/api/approvals/resolve", json={"id": "approval-7", "resolution": "approved"})

    assert response.status_code == 200
    assert response.json()["act_id"] == "ACT-SUP"
    execute_supervisor.assert_awaited_once_with(
        "approval-7",
        "command-center",
        "run_approved",
