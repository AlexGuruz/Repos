...
async def _execute_approved(approval_id: str, spec: dict):
    ...
    if "payload" in spec and action:
        try:
            from services.supervisor_bridge import CONTROLLED_OPS, execute_approved as execute_supervisor_approved
        except Exception:
            CONTROLLED_OPS = frozenset()
            execute_supervisor_approved = None
        if execute_supervisor_approved and action in CONTROLLED_OPS:
            return await execute_supervisor_approved(
                approval_id,
                str(spec.get("agent") or "orchestrator"),
                action,
