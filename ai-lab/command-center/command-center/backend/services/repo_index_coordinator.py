...
class RepoIndexCoordinator:
    ...
    def _persist_state(self) -> None:
        out = {rid: asdict(st) for rid, st in self._states.items()}
    ...
    async def stop(self) -> None:
        self._running = False
        # Persist on stop so restart continuity is acceptable.
    ...
    async def _handle_build_failure(self, repo_id: str, error: str) -> None:
        async with self._lock:
            ...
            self._persist_state()
        await bus.publish("repo_index_build_failed", {"repo_id": repo_id, "error": str(error), "timestamp": _now_iso()})
