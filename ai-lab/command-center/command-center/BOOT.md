...
**Path:** Scripts are **not** in the `ai-lab` repo root. They live here:

`E:\Repos\ai-lab\command-center\command-center\scripts\`

If you run `.\scripts\start.ps1` from `E:\Repos\ai-lab`, PowerShell will say the script was not found — **`cd` into the folder below first.**

## Option A: Script (one command)

From the **inner** `command-center` app folder (the one that contains `scripts\`, `backend\`, `frontend\`):
...
.\scripts\restart.ps1       # stop, wait, then start.ps1 again
.\scripts\restart.ps1 -LocalOnly
```
...
2. In the frontend folder, clean and reinstall:
   ...
   npm install
   ```
3. Run `.\scripts\start.ps1 -LocalOnly` again from `command-center\command-center`.
...
2. **Or install Python 3.12** and use it for deps only:
   ...
   ```
   Then run `.\scripts\deps.ps1` (creates `.venv` with 3.12) and start backend/frontend manually, or run `.\scripts\start.ps1 -LocalOnly` again.
...
- **Normal behavior:** JSON with `services[]`, `tunnel_status`, `all_ok`, `last_checked`. If `brain` cannot load, you still get JSON with an `error` field (not a dropped connection).
