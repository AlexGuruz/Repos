      ...
      {workersHealth && (
        <div className="px-3 py-3 border-t border-white/8">
          <div className="flex items-center gap-2 flex-wrap">
            <SectionLabel>Worker services · {workersHealth.worker_name ?? 'worker-rig-01'}</SectionLabel>
          ...
          {workersHealth.tunnel_status && (
            <div className="flex items-center gap-2 mt-1.5 text-[10px] text-white/45">
              ...
              />
              <span>Tunnel: {workersHealth.tunnel_status.likely_up ? 'up' : 'down'}</span>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {workersHealth.services?.map(s => (
              <span
                ...
                title={[s.detail, s.latency_ms != null ? `${s.latency_ms.toFixed(0)} ms` : ''].filter(Boolean).join(' · ')}
              >
