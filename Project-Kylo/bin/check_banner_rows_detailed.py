...
def check_banner_rows_detailed():
    """Check for banner rows in a larger range"""
    ...
    print("=== Detailed Banner Row Check ===\n")
    ...
    for company in cfg.get("companies", []):
        ...
        try:
            # Check pending tab for banner rows in a larger range
            pending_title = f"{company_id} Pending"
            
            # Check first 100 rows for banner rows
            try:
                data_result = service.spreadsheets().values().get(
                    spreadsheetId=spreadsheet_id,
                    range=f"{pending_title}!A1:A100"
                ).execute()
                ...
                if banner_rows:
                    ...
                    if len(data_rows) > 1:
                        ...
                        for i in range(max(1, len(data_rows) - 5), len(data_rows)):
                            if data_rows[i]:
