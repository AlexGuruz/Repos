...
# Allow importing Project-Kylo services config when running from scripts/
...
class GoogleSheetsSync:
    ...
    def __init__(self, credentials_path: str = None, dry_run: bool = False, verbose: bool = False):
        ...
        """
        # Resolve credentials path with sensible precedence:
        #   1) explicit --credentials (if exists)
        #   2) GOOGLE_APPLICATION_CREDENTIALS env var
        #   3) Project-Kylo unified config (config/kylo.config.yaml)
        #   4) legacy scripts/config/service_account.json (old ScriptHub layout)
        def _resolve_creds(initial: Optional[str]) -> str:
            if initial:
                try:
                    if os.path.exists(str(initial)):
                        return str(initial)
                except Exception:
                    pass
            ...
            try:
                ...
                cfg = load_config()
                cfg_path = (cfg.get("google.service_account_json_path") or "").strip()
                if cfg_path and os.path.exists(cfg_path):
                    return cfg_path
            except Exception:
                pass
            script_dir = Path(__file__).parent.parent
            return str(script_dir / CONFIG["credentials_file"])

        credentials_path = _resolve_creds(credentials_path)
