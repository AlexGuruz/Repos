...
.DESCRIPTION
  Runs ssh -L 8765:127.0.0.1:8765 worker@192.168.40.25 -N in the background (127.0.0.1 for IPv4 on worker). Optionally use -Install
  to register a scheduled task that starts the tunnel at user logon (auto-reconnect after reboot).
.PARAMETER SshTarget
  SSH target (user@host). Default: worker@192.168.40.25 (worker-node LAN IP).
.PARAMETER LocalPort
