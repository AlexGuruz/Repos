...
def _post_success_key(source_sid: str, src_tab: str, row0: int, target_a1: str) -> Tuple[str, str, int, str]:
    return (
        str(source_sid or ""),
        str(src_tab or "TRANSACTIONS"),
        int(row0),
        str(target_a1 or ""),
    )
...
def run(company: str, *, baseline: bool = False, verify: Optional[bool] = None, rules_changed: bool = False):
    ...
    def _process_target(target_sid: str, txns_for_target: List[dict], ignore_posted: bool = False) -> Dict[str, object]:
        ...
        post_meta_by_success_key: Dict[Tuple[str, str, int, str], Dict[str, Any]] = {}
        ...
        for (tab, header, date_key, amount_cents, src_tab, row0, txn_uid, source_sid, posted_date, description, for_marking) in matched_writes:
            ...
            if for_marking:
                ...
                post_meta_by_success_key[_post_success_key(source_sid, src_tab, row0, a1)] = {
                    "txn_uid": txn_uid,
        ...
        if not dry_run and bool(cfg.get("posting.mark_posted", True)):
            ...
            if record_successful_post and instance_id:
                for (source_sid, src_tab, row0, msg, target_a1) in success_notes:
                    if target_a1 not in posted_ok_ranges:
                        continue
                    meta = post_meta_by_success_key.get(_post_success_key(source_sid, src_tab, row0, target_a1)) or {}
                    try:
                        record_successful_post(
                            instance_id=instance_id,
