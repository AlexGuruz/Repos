...
def run(company: str, *, baseline: bool = False, verify: Optional[bool] = None, rules_changed: bool = False):
    ...
    def _process_target(target_sid: str, txns_for_target: List[dict], ignore_posted: bool = False) -> Dict[str, object]:
        ...
        success_notes: List[Tuple[str, str, int, str, str, Dict[str, Any]]] = []  # source row + note + per-row post metadata
        ...
        return {
            ...
            "failed_target_ranges": sorted(failed_ranges),
            "failed_target_range_count": int(len(failed_ranges)),
    ...
    for target_sid, txns_for_target in txns_by_target_sid.items():
        ...
        total_failed_target_ranges += int(result.get("failed_target_range_count", 0) or 0)
        any_target_failure = any_target_failure or bool(result.get("error"))
    ...
    return {
        ...
        "failed_target_range_count": int(total_failed_target_ranges),
        "error": bool(any_target_failure),
    }
