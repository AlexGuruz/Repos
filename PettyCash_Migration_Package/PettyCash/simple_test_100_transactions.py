#!/usr/bin/env python3
"""
Simple Test - First 100 Transactions
Test rule matching without layout map issues
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/simple_test_100.log"),
        logging.StreamHandler()
    ]
)

def simple_test_100_transactions():
    """Simple test with first 100 transactions."""
    print("SIMPLE TEST - FIRST 100 TRANSACTIONS")
    print("=" * 80)
    ...
    try:
            ...
            if match_result:
                ...
                if len(matched_examples) < 5:
                    matched_examples.append({
                        ...
                        'target_sheet': match_result['target_sheet'],
                        'target_header': match_result['target_header'],
                ...
                db_manager.update_transaction_with_match(
                    ...
                    match_result['target_sheet'],
                    match_result['target_header'],
        ...
        if matched_examples:
            ...
            for i, example in enumerate(matched_examples, 1):
                print(f"  {i}. '{example['source']}' -> {example['target_sheet']}/{example['target_header']} (Confidence: {example['confidence']})")
