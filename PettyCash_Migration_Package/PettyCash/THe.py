...
from real_time_monitor_enhanced import EnhancedRealTimeMonitor as RealTimeMonitor
...
class PettyCashSorterFinal:
    ...
    def __init__(self, config_file: str = "config/system_config.json"):
        ...
        if self.config_manager.get('monitoring.real_time_dashboard.enabled', False):
            try:
                ...
                # Set AI matcher reference for rule management
                if hasattr(self.real_time_monitor, 'set_ai_matcher'):
                    self.real_time_monitor.set_ai_matcher(self.ai_matcher)
    ...
    def download_and_process_transactions(self) -> Optional[List[Dict]]:
        """Download fresh transactions from Google Sheets and identify new ones."""
        try:
            logging.info("📥 Downloading fresh transactions from Google Sheets...")
            
            # Start progress monitoring
            self.progress_monitor.start_operation(
                "Transaction Download",
                0,  # Will update with actual count
                "Downloading fresh petty cash transactions from Google Sheets"
            )
            
            # Download fresh data from Google Sheets
            fresh_transactions = self.csv_downloader.download_petty_cash_data()
            if not fresh_transactions:
                logging.warning("No fresh transactions downloaded from Google Sheets")
                return []
            
            # Update progress monitor with actual count
            self.progress_monitor.total_items = len(fresh_transactions)
            self.progress_monitor.update_progress(0, f"Downloaded {len(fresh_transactions)} fresh transactions")
            
            logging.info(f"📊 Downloaded {len(fresh_transactions)} fresh transactions from Google Sheets")
            
            # The hash deduplication is already handled in csv_downloader.download_petty_cash_data()
            # So fresh_transactions already contains only new transactions
            new_transactions = fresh_transactions
            
            self.progress_monitor.update_progress(
                len(fresh_transactions),
                f"Found {len(new_transactions)} new transactions to process"
            )
            ...
            if self.real_time_monitor:
                try:
                    self.real_time_monitor.update_transaction_count(len(fresh_transactions))
                    self.real_time_monitor.update_new_transaction_count(len(new_transactions))
                    self.real_time_monitor.add_log_entry('INFO', f'Downloaded {len(fresh_transactions)} fresh transactions, {len(new_transactions)} new')
                except Exception as monitor_error:
            ...
            logging.error(f"Error downloading fresh transactions from Google Sheets: {e}")
            return None
