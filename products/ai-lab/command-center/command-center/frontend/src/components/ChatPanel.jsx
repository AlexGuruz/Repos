...
function isAprStub(ev) {
  return String(ev?.id || '').toUpperCase().startsWith('APR-')
}

function isBrainExecuteApproval(ev) {
  if (!String(ev?.id || '').startsWith('approval-')) return false
  const tool = ev.tool_name || ev.payload?.tool_name
  return Boolean(tool) || ev.execute_path === true
}
...
function ApprovalCard({ ev }) {
  ...
  const aprStub = isAprStub(ev)
  const brainExec = isBrainExecuteApproval(ev)
  const toolName = ev.tool_name || ev.payload?.tool_name || null
  ...
  async function handleAddPermanent() {
    if (!canPermanent || permBusy || busy) return
    setPermBusy(true)
    setBusy(true)
    try {
      // permanent-before-resolve: save rule while still pending, then resolve.
      const body =
      ...
      if (!res?.ok) {
        throw new Error(res?.error || res?.detail || 'Could not save permanent rule')
      }
      const rid = res?.rule?.id || 'rule'
      const resolveRes = await api.resolveApproval(ev.id, 'approved')
      if (!resolveRes?.ok) {
        throw new Error(resolveRes?.error || 'Permanent rule saved but resolve failed')
      }
      resolve(ev.id, 'approved')
      addMsg({
        role: 'ai',
        text: `Always Approve: saved permanent rule **${rid}** for \`${ev.action}\` and approved ${ev.id}. Matching future requests auto-approve.`,
      })
    } catch (e) {
      addMsg({ role: 'ai', text: `Always Approve failed for ${ev.id}: ${e.message}` })
    } finally {
      setPermBusy(false)
      setBusy(false)
    }
  ...
  async function handleResolve(resolution) {
    ...
    try {
      if (ev.action === 'retail_capital_scenario') {
        ...
        const res = await api.resolveApproval(ev.id, resolution)
        if (!res?.ok) {
          throw new Error(res?.error || `Resolve failed for ${ev.id}`)
        }
        resolve(ev.id, resolution)
        if (aprStub || res.dismissed_only) {
          addMsg({ role: 'ai', text: `${ev.id} dismissed (APR stub - no execute path).` })
        } else {
          const execNote =
            resolution === 'approved'
              ? (res.executed
                  ? (res.execute_queued
                      ? `Decision recorded; execution queued.`
                      : (res.execute_ok === false
                          ? `Decision recorded; execution reported an error.`
                          : `Decision recorded; execute path notified.`))
                  : `Decision recorded (no tool_name to execute).`)
              : 'Denied - no execute.'
          addMsg({ role: 'ai', text: `${ev.id} ${resolution}. ${execNote}` })
        }
      }
    } catch (e) {
  ...
  const border = aprStub ? 'rgba(148,163,184,0.35)' : 'rgba(245,158,11,0.35)'
  const bg = aprStub ? 'rgba(148,163,184,0.04)' : 'rgba(245,158,11,0.04)'

  return (
    <div className="rounded-lg border mx-1 p-3" style={{ borderColor: border, background: bg }}>
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        <div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0" style={{ background: aprStub ? 'rgba(148,163,184,0.15)' : 'rgba(245,158,11,0.15)' }}>
          <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke={aprStub ? '#94a3b8' : '#fcd34d'} strokeWidth="1.5">
            <path d="M8 2v6M8 12v.5" strokeLinecap="round"/><circle cx="8" cy="8" r="6.5"/>
          </svg>
        </div>
        <span className="text-[12px] font-medium text-white/80">
          {aprStub ? 'APR stub' : brainExec ? 'Execute approval' : 'Approval request'}
        </span>
        {aprStub && (
          <span
            className="text-[9px] font-mono px-1.5 py-0.5 rounded border"
            style={{ borderColor: 'rgba(148,163,184,0.4)', color: '#cbd5e1' }}
            title="Supervisor WS stub - dismiss only. Real execute path is brain approval-N with tool_name."
          >
            dismiss only
          </span>
        )}
        {brainExec && toolName && (
          <span
            ...
            title="Brain queue execute path"
          >
            tool:{toolName}
          </span>
        )}
      ...
      <div className="text-[11px] text-white/55 leading-relaxed mb-2.5">
        ...
        {' '}via <code className="text-[10px] px-1 py-0.5 rounded" style={{ background: aprStub ? 'rgba(148,163,184,0.12)' : 'rgba(245,158,11,0.1)', color: aprStub ? '#cbd5e1' : '#fcd34d' }}>{ev.agent}</code>
        <br />{ev.detail}
        {aprStub ? (
          <div className="text-[10px] text-white/35 mt-1">Not an execute path. Use Sidebar "Dismiss APR stubs" or Dismiss here.</div>
        ) : null}
      </div>
      {ev.catalog_context ? (
        <pre
      ...
      {isPending ? (
        aprStub ? (
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => handleResolve('denied')} disabled={busy}
              className="px-3 py-1 text-[11px] rounded transition-colors disabled:opacity-40"
              style={{ background: 'rgba(148,163,184,0.1)', border: '0.5px solid rgba(148,163,184,0.35)', color: '#cbd5e1' }}
              title="Dismiss APR-* supervisor stub (no tool execute)">
              {busy ? '…' : 'Dismiss'}
            </button>
          </div>
        ) : (
            ...
            <button
              ...
              title={
                !canPermanent
                  ...
                  : 'Save permanent scoped allowlist rule then approve (permanent-before-resolve)'
              }
              className="px-3 py-1 text-[11px] rounded transition-colors disabled:opacity-40"
              style={{
...
export default function ChatPanel() {
  ...
  // Pending-only. Prefer brain approve→execute cards; demote APR-* stubs.
  const pendingApprovals = events.filter(
    e =>
      e.type === 'approval' &&
      e.status === 'pending' &&
      e.action !== 'retail_capital_scenario',
  )
  const executeApprovals = pendingApprovals.filter(e => isBrainExecuteApproval(e) || (!isAprStub(e) && String(e.id || '').startsWith('approval-')))
  const aprStubs = pendingApprovals.filter(e => isAprStub(e))
  const otherApprovals = pendingApprovals.filter(e => !executeApprovals.includes(e) && !aprStubs.includes(e))
  const ranked = [...executeApprovals, ...otherApprovals, ...aprStubs]
  const visibleApprovals = ranked.slice(0, 8)
  const hiddenApprovalCount = Math.max(0, ranked.length - visibleApprovals.length)
  const hiddenAprOnly = hiddenApprovalCount > 0 && executeApprovals.length + otherApprovals.length <= 8

  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="px-4 py-2 border-b border-white/8 text-[11px] text-white/45">
        Operator routing and approvals only. This tab does not replace Cursor — use Cursor for editing and agent work on Acheron.
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3">
        {messages.map((msg, i) => <Message key={msg.id || `i-${i}`} msg={msg} />)}
        {visibleApprovals.map(ev => (
          <ApprovalCard key={ev.id} ev={ev} />
        ))}
        {hiddenApprovalCount > 0 && (
          <div className="text-[11px] text-white/35 px-1">
            +{hiddenApprovalCount} more pending
            {hiddenAprOnly || aprStubs.length > 0
              ? ` (${aprStubs.length} APR stubs - dismiss-only; Sidebar can clear)`
              : ''}
          </div>
        )}
