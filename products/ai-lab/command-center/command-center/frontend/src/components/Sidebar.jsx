...
function isAprStubId(id) {
  return String(id || '').toUpperCase().startsWith('APR-')
}
...
export default function Sidebar({ onSelect }) {
  ...
  const { events, pendingCount, resolveApproval } = useEventStore()

  const filtered = tab === 'all' ? events
    : events.filter(e => e.type === tab)

  const pendingAprStubs = useMemo(
    () => events.filter(e => e.type === 'approval' && e.status === 'pending' && isAprStubId(e.id)),
    [events],
  )

  async function dismissAllAprStubs() {
    if (dismissBusy || pendingAprStubs.length === 0) return
    setDismissBusy(true)
    try {
      // APR-* is dismiss-only (no execute). Brain approval-N with tool_name is left alone.
      for (const ev of pendingAprStubs) {
        try {
          const res = await api.resolveApproval(ev.id, 'denied')
          if (res?.ok) resolveApproval(ev.id, 'denied')
        } catch {
          /* continue remaining stubs */
        }
      }
    } finally {
  ...
  return (
    <div className="flex flex-col border-r border-white/8" style={{ width: 268, minWidth: 268, background: 'rgba(255,255,255,0.025)' }}>
      ...
      {tab === 'approval' && pendingAprStubs.length > 0 && (
        <div className="px-3 py-2 border-b border-white/8 flex flex-col gap-1">
          <button
            type="button"
            disabled={dismissBusy}
            onClick={dismissAllAprStubs}
            className="text-[10px] px-2 py-1 rounded border border-amber-500/40 text-amber-200/90 hover:bg-amber-500/10 disabled:opacity-50"
            title="Dismiss supervisor APR-* stubs only. Does not touch brain approval-N execute queue."
          >
            {dismissBusy ? 'Dismissing…' : `Dismiss APR stubs (${pendingAprStubs.length})`}
          </button>
          <div className="text-[9px] text-white/30">
            APR-* = dismiss only. Real execute path = brain <span className="font-mono">approval-N</span> with tool_name.
          </div>
        </div>
      )}
