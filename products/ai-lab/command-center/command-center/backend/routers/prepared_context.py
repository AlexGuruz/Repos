...
from services.executors import CONTEXT_EXECUTOR, run_in
...
async def prepared_context_all():
    return await run_in(CONTEXT_EXECUTOR, _load_all)


@router.get("/api/prepared-context/{snapshot_type}")
async def prepared_context_one(snapshot_type: str):
    data = await run_in(CONTEXT_EXECUTOR, _load_one, snapshot_type)
...
async def refresh_prepared_context(snapshot_type: str):
    return await run_in(CONTEXT_EXECUTOR, _refresh, snapshot_type)
...
async def growflow_validation_status():
    ...
    # Dual-read: products/growflow (zoned) then legacy flat Growflow under monorepo root.
    growflow_root = None
    for candidate in (
        AI_LAB_ROOT.parent / "growflow",
        AI_LAB_ROOT.parent / "Growflow",
        AI_LAB_ROOT.parent.parent / "products" / "growflow",
        AI_LAB_ROOT.parent.parent / "Growflow",
    ):
        if candidate.is_dir():
            growflow_root = candidate
            break
    if growflow_root is None:
