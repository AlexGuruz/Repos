...
def _channel_patches():
    return (
        patch("services.supervisor_bridge.channels.control.publish", new_callable=AsyncMock),
        patch("services.supervisor_bridge.channels.ops.publish", new_callable=AsyncMock),
    )


def test_supervisor_controlled_op_queues_approval() -> None:
    p1, p2 = _channel_patches()
    with p1, p2:
        client = _client()
...
def test_supervisor_blocks_read_op_extension_without_metadata() -> None:
    p1, p2 = _channel_patches()
    with patch("services.supervisor_bridge.settings.worker_read_ops_extra", "mystery_read"), p1, p2:
        client = _client()
