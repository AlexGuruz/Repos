...
def _resolve_repo_id_path(rid: str) -> str:
    """Map repo_id to an absolute path under zoned products/ with legacy dual-read."""
    from core.ai_lab import AI_LAB_ROOT

    products = AI_LAB_ROOT.parent  # .../products when AI_LAB_ROOT is products/ai-lab
    monorepo = products.parent if products.name.lower() == "products" else products
    # Legacy flat names -> current product folder ids
    aliases = {
        "Growflow": "growflow",
        "growflow": "growflow",
        "Project-Kylo": "project-kylo",
        "project-kylo": "project-kylo",
        "Greg-Kylo": "greg-kylo",
        "greg-kylo": "greg-kylo",
        "ai-lab": "ai-lab",
        "ai-lab-governance": "ai-lab-governance",
        "cog-allocation-system": "cog-allocation",
        "cog-allocation": "cog-allocation",
        "kylo-site": "kylo-site",
        "geomapper app": "gigatt-geomapper",
        "gigatt-geomapper": "gigatt-geomapper",
    }
    product_id = aliases.get(rid, rid)
    for candidate in (
        products / product_id,
        products / rid,
        monorepo / "products" / product_id,
        monorepo / rid,  # legacy flat
    ):
        try:
            if candidate.is_dir():
            ...
            continue
    # Prefer zoned target even if missing (worker may create/check)
    return str((products / product_id).resolve())
...
def _normalize_worker_post_body(op: str, payload: dict) -> dict:
    ...
    Map repo_id -> absolute path under products/ (zoned) with legacy flat dual-read.
    """
    ...
    if not rid:
        return body
    try:
        body["repo_path"] = _resolve_repo_id_path(rid)
    except Exception:
        body["repo_path"] = rid
    return body


async def _worker_call_direct(op: str, payload: dict) -> dict:
    """Low-level HTTP call to the worker tunnel (scheduled via TunnelScheduler)."""
...
async def _worker_call(op: str, payload: dict) -> tuple[dict, str | None]:
    """Forward a read-only op through the fair TunnelScheduler. Returns (result, job_id)."""
    lane = tunnel_scheduler.lane_for_op(op)
    timeout_sec = _worker_timeout_seconds(op)
    job_ids: list[str] = []

    async def _factory():
        return await _worker_call_direct(op, payload)

    result = await tunnel_scheduler.submit(
        lane,
        _factory,
        timeout=timeout_sec + 5.0,
        op=op,
        on_enqueued=job_ids.append,
    )
    return result, (job_ids[0] if job_ids else None)
...
async def fetch_worker_repo_status(repo_id: str) -> dict | None:
    ...
    try:
        return await tunnel_scheduler.submit(
            "control",
            _factory,
            timeout=float(settings.worker_bridge_timeout_seconds) + 5.0,
            op="repo_status",
        )
    except Exception:
        return None
...
async def _log_action(action_id: str, agent: str, op: str, detail: str, status: str = "done"):
    ...
    await channels.control.publish("action", ev.model_dump())
...
async def route_intent(agent: str, op: str, payload: dict) -> dict:
    ...
    if op in effective_allowlisted_read_ops():
        await channels.ops.publish("feed", {
        ...
        try:
            result, tunnel_job_id = await _worker_call(op, payload)
            await _log_action(act_id, agent, op, f"{op} completed ({settings.worker_tunnel_url})")
            out = {"ok": True, "act_id": act_id, "result": result}
            if tunnel_job_id:
                out["tunnel_job_id"] = tunnel_job_id
            return out
    ...
    if op in CONTROLLED_OPS:
        ...
        await channels.control.publish("approval", apr.model_dump(mode="json"))
