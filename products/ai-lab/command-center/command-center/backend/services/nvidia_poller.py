...
from services.executors import TELEMETRY_EXECUTOR, run_in
...
async def nvidia_poll_loop():
    """Background task — runs forever, publishes hardware snapshots on telemetry channel."""
    interval = max(1, int(settings.nvidia_smi_poll_interval or 5))
    while True:
        # Never run NVML/psutil on the event loop — that wedged Approve/Deny HTTP.
        try:
            payload, alerts = await asyncio.wait_for(
                run_in(TELEMETRY_EXECUTOR, _hardware_snapshot_and_alerts),
                timeout=4.0,
            )
        except Exception:
            payload, alerts = _fallback_snapshot(), []
        await channels.telemetry.publish("hardware", payload)
        if alerts:
            await channels.telemetry.publish(
                "hardware_alert",
                {"alerts": alerts, "snapshot": payload, "timestamp": payload.get("timestamp")},
            )
