# archive_kylo_simplified_execute.ps1
# Archive Execute Script for Kylo-Simplified (requires -Execute flag)
# Reads archive_manifest.v1.csv and performs actual moves/copies
...
$LogFile = "D:\_PORTABLE_CONTROL\_logs\archive_kylo_simplified_execute.log"
...
if (-not $Execute) {
    ...
    Write-Host ""
    Write-Host "  .\archive_kylo_simplified_execute.ps1 -Execute" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Running dry-run instead..." -ForegroundColor Yellow
    Write-Host ""
    
    # Run dry-run script instead
    & "D:\_PORTABLE_CONTROL\_scripts\archive_kylo_simplified_dryrun.ps1" -ShowLog:$ShowLog
    exit 0
}
...
Write-Log "================================================" "INFO"
Write-Log "Kylo-Simplified Archive Execute Script Started" "INFO"
Write-Log "================================================" "INFO"
    ...
    try {
        switch ($Action.ToLower()) {
            "move" {
                ...
                $Stats.Move++
                Write-Log "  SUCCESS: Moved directory" "INFO"
            }
            "copy" {
                Write-Log "  EXECUTING COPY: $SourcePath -> $DestPath" "WARN"
                Copy-Item -Path $SourcePath -Destination $DestPath -Recurse -ErrorAction Stop
                $Stats.Copy++
                Write-Log "  SUCCESS: Copied directory" "INFO"
            }
            "archive_only" {
